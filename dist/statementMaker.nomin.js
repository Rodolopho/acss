...
// EXTERNAL MODULE: ./src/static/customColor.js
var static_customColor = __webpack_require__(1);
...
var statementMaker = {
  ...
  custom: {},
  addCustom: function addCustom(name, obj) {
  ...
  handleSuffix: function handleSuffix(classname) {
    var match = /[-_]([id])$/;
  ...
  getPropertyAndValue: function getPropertyAndValue(classname) {
    var result = this.make(classname, null, true);
    if (!result[0] && !result[1]) return result[2];
  },
  make: function make(classname, as, bool) {
    //reset 
    var _ref = [null, null, null, null, null];
    this.device = _ref[0];
    this.classname = _ref[1];
    this.selctor = _ref[2];
    this.propertyAndValue = _ref[3];
    this.hasSuffix = _ref[4];
    this.classname = as ? as : classname; // -------------------------responsive----------------
    ...
    var selectorResult = whileMatchNCall(classname);
    this.selector = selectorResult[1];
    classname = selectorResult[0].replace(/^[_-]/, ""); //------------suffix-flag------

    classname = this.handleSuffix(classname); //------------proverty and value---------------
    ...
    var statement = "." + this.classname + this.selector + "{" + this.propertyAndValue + (this.hasSuffix ? this.hasSuffix : '') + " ;}";
  ...
  extend: function extend(a, b) {
    if (_typeof(a) === 'object') {
      for (var key in a) {
        this.cache.propertyAndValue[key] = a[key]; // if(this.cachstatementMaker.staticClassNames.hasOwnProperty(key)){
  ...
  styleSheetCompiler: function styleSheetCompiler(content) {
    var _this = this;
    ...
    var m4 = /(?<=[\/][\s]*)([A-Za-z0-9-_]+)(?=[\s]*[;|}])/g;
    var m5 = /(?<=[\{][\s]*)([A-Za-z0-9-_]+)(?=[\s]*[}])/g;
    return content.replace(match, function (e) {
      //1.repalce {.....;
      e = e.replace(m1, function (m) {
        var result = _this.getPropertyAndValue(m);
  ...
  groupForJs: function groupForJs(string, bool) {
    var _this2 = this;
    ...
    list.forEach(function (e) {
      ...
      if (_this2.cache.propertyAndValue.hasOwnProperty(e)) {
        pNv = _this2.cache.propertyAndValue[e];
      } else {
        pNv = propertyAndValue(e, _this2.custom);
        if (pNv) _this2.cache.propertyAndValue[e] = pNv;
      }

      if (!pNv) return "";
      var result = pNv.split(":");

      if (result.length === 2) {
        var key = result[0].replace(/-([a-z])/g, function (e, a) {
          return a.toUpperCase();
        });
        var value = bool === true ? result[1].replace(/px$/g, "") : result[1];
  ...
  groupForStyle: function groupForStyle(str) {
    var _this3 = this;
    ...
    list.forEach(function (e) {
      ...
      if (_this3.cache.propertyAndValue.hasOwnProperty(e)) {
        container += _this3.cache.propertyAndValue[e] + (_this3.hasSuffix ? _this3.hasSuffix : '') + " ;\n";
      } else {
        var pNv = propertyAndValue(e, _this3.custom);

        if (pNv) {
          _this3.cache.propertyAndValue[e] = pNv;
  ...
  fromString: function fromString(string, group) {
    var _this4 = this;
    ...
    var classList = string.split(/\s+/);
    classList.forEach(function (eachClass) {
      var result = _this4.make(eachClass);
  ...
  group: function group(str, as) {
    ...
    list.forEach(function (e) {
      if (matcher.selector.match.test(e) || matcher.device.match.test(e)) {
        var result = _this5.make(e, as);

        if (result) statement += result + "\n";
      } else {
        _this5.hasSuffix = null;
        e = _this5.handleSuffix(e);

        if (_this5.cache.propertyAndValue.hasOwnProperty(e)) {
          container += _this5.cache.propertyAndValue[e] + (_this5.hasSuffix ? _this5.hasSuffix : '') + " ;\n";
        } else {
          var pNv = propertyAndValue(e, _this5.custom);

          if (pNv) {
            _this5.cache.propertyAndValue[e] = pNv;
            container += pNv + (_this5.hasSuffix ? _this5.hasSuffix : '') + " ;\n";
          }
        }
      }
    });
    return "".concat(statement, " \n .").concat(as, " { ").concat(container, " }");
  }
};
statementMaker.addCustom('color', static_customColor["a" /* default */]);
// CONCATENATED MODULE: ./src/classPrinter.js
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return classPrinter_classPrinter; });
 
 /* harmony default export */ var classPrinter = __webpack_exports__["b"] = (statementMaker);
...
/* harmony default export */ __webpack_exports__["a"] = (customColor);
...
/* WEBPACK VAR INJECTION */(function(module) {/* harmony import */ var _src_classPrinter_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(0);





module.exports.statementMaker=_src_classPrinter_js__WEBPACK_IMPORTED_MODULE_0__[/* default */ "b"];
